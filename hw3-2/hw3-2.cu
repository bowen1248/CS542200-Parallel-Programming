...
__device__ void block_APSP(unsigned int *C, unsigned int *A, unsigned int *B, int x, int y) {
    for (int k = 0; k < BLOCK_SIZE; k++) {
        // printf("%d %d %d %d %d %d\n", blockIdx.y, blockIdx.x, y, x, A[y * BLOCK_SIZE + k], B[k * BLOCK_SIZE + x]);
        for (int i = y; i < BLOCK_SIZE; i += 32) {
            for (int j = x; j < (BLOCK_SIZE / 2); j += 32) {
                C[i * (BLOCK_SIZE / 2) + j] = __vmins2(__vadd2(A[i * (BLOCK_SIZE / 2) + k], B[k * (BLOCK_SIZE / 2) + j]), C[i * (BLOCK_SIZE / 2) + j]);
...
__global__ void stage1(unsigned int *devMat, int startIdx, int n) {
    __shared__ unsigned int mat[BLOCK_SIZE * BLOCK_SIZE / 2];
    ...
    for (int i = threadIdx.y; i < BLOCK_SIZE; i += blockDim.y) {
        for (int j = threadIdx.x; j < (BLOCK_SIZE / 2); j += blockDim.x) {
            mat[i * (BLOCK_SIZE / 2) + j] = devMat[(startIdx + i) * (n / 2) + (startIdx + j)];
        }
    }
    __syncthreads();
    ...
    for (int i = threadIdx.y; i < BLOCK_SIZE; i += blockDim.y) {
        for (int j = threadIdx.x; j < (BLOCK_SIZE / 2); j += blockDim.x) {
            devMat[(startIdx + i) * (n / 2) + (startIdx + j)] = mat[i * (BLOCK_SIZE / 2) + j];
        }
    }
...
int main(int argc, char **argv) {
    ...
    unsigned int* device_adjMat;
    cudaMalloc((void **) &device_adjMat, n * n * sizeof(unsigned int) / 2);

    // Put adjanency matrix to GPU
    cudaMemcpy(device_adjMat, adjMat, n * n * sizeof(unsigned int) / 2, cudaMemcpyHostToDevice);
    ...
    // output
    cudaMemcpy(adjMat, device_adjMat, n * n * sizeof(unsigned int) / 2, cudaMemcpyDeviceToHost);
